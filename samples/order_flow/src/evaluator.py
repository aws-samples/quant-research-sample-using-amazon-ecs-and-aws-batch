...
class ModelEvaluator:
    ...
    def evaluate_model(self, model: nn.Module, dataset, split: str = 'test', 
        ...
        logger.info(f"Using evaluation batch size: {batch_size} for faster {split} evaluation")
        ...
        metrics = self._calculate_metrics(all_targets, all_predictions, all_probabilities, inference_times)
        
        # Log summary
        logger.info(f"✓ {split.capitalize()} evaluation complete:")
        logger.info(f"  Accuracy: {metrics['accuracy']:.4f}")
        logger.info(f"  F1 Score: {metrics['f1_macro']:.4f}")
        logger.info(f"  Inference: {metrics['avg_inference_time']*1000:.2f}ms avg, {metrics['inference_samples_per_second']:.0f} samples/sec")
        
        return metrics
    
    def _calculate_metrics(self, targets, predictions, probabilities, inference_times) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'inference_samples_per_second': len(predictions) / np.sum(inference_times) if np.sum(inference_times) > 0 else 0
        }
        ...
        for class_idx, class_name in enumerate(self.class_names):
            ...
            if np.sum(class_mask) > 0:
                class_precision = precision_score(targets, predictions, labels=[class_idx], average=None, zero_division=0)
                class_recall = recall_score(targets, predictions, labels=[class_idx], average=None, zero_division=0)
                class_f1 = f1_score(targets, predictions, labels=[class_idx], average=None, zero_division=0)
                
                metrics[f'precision_{class_name}'] = class_precision[0] if len(class_precision) > 0 else 0.0
                metrics[f'recall_{class_name}'] = class_recall[0] if len(class_recall) > 0 else 0.0
                metrics[f'f1_{class_name}'] = class_f1[0] if len(class_f1) > 0 else 0.0
        
        # Multi-class AUC
        try:
            metrics['auc_macro'] = roc_auc_score(targets, probabilities, multi_class='ovr', average='macro')
    ...
    def show_class_distributions(self, datasets: Dict[str, Any]):
        ...
        for split_name, dataset in datasets.items():
            ...
            for class_idx, count in zip(unique_classes, class_counts):
                class_name = class_names[int(class_idx)] if class_idx < len(class_names) else f'Class {int(class_idx)}'
            ...
            if len(class_counts) > 1:
                ...
                if split_name == 'train' and imbalance_ratio < 2.0:
                    logger.info(f"  ✓ Training set appears to be undersampled (low imbalance ratio)")
                elif split_name == 'train' and imbalance_ratio > 10.0:
                    logger.warning(f"  ⚠ Training set appears highly imbalanced (high imbalance ratio)")
        ...
        if 'train' in datasets and 'validation' in datasets and 'test' in datasets:
            ...
            logger.info(f"  Validation: {val_size:,} ({val_size/train_size:.1f}x larger)")
            logger.info(f"  Test: {test_size:,} ({test_size/train_size:.1f}x larger)")
            
            if val_size > train_size * 10 and test_size > train_size * 10:
                logger.info(f"  ✓ Training set is significantly smaller - undersampling was applied")
            else:
                logger.info(f"  ℹ Training set size is similar to other splits - no undersampling detected")
        
        logger.info("="*60)
    
    def print_detailed_classification_report(self, targets, predictions, split_name: str = 'test'):
        """Print detailed classification report."""
        logger.info(f"\n{split_name.upper()} SET - DETAILED CLASSIFICATION REPORT:")
        logger.info("="*50)
        
        # Classification report
        report = classification_report(targets, predictions, target_names=self.class_names, zero_division=0)
        logger.info(f"\n{report}")
...
def evaluate_model(model: nn.Module, dataset, device: str = 'cpu', 
                  split: str = 'test', batch_size: int = 512) -> Dict[str, Any]:
