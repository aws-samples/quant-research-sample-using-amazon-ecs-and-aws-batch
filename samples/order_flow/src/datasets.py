...
class PolarSequenceDataset(Dataset):
    ...
    def __init__(self, df: pl.DataFrame, features: List[str], target: str, sequence_length: int = 128, device: str = 'cpu'):
        self.df = df
        ...
        logger.info(f"✓ Created Polars sequence dataset with {self.length:,} sequences (memory efficient)")
    ...
    def __getitem__(self, idx):
        ...
        # Convert to tensors
        sequence_tensor = torch.tensor(sequence_data, dtype=torch.float32, device=self.device)
...
class OptimizedSequenceDataset(Dataset):
    ...
    def __init__(self, device, dataframe, target, features, sequence_length=128, method='strided'):
        self.features = features
        self.target = target
        self.sequence_length = sequence_length
        self.device = device
        ...
        feature_data = torch.tensor(dataframe[features].values, dtype=torch.float32, device='cpu')
        target_data = torch.tensor(dataframe[target].values, dtype=torch.long, device='cpu')
        ...
        logger.info(f"✓ Memory usage: {self.X.numel() * 4 / 1024**3:.2f} GB (sequences) + {self.y.numel() * 8 / 1024**3:.3f} GB (targets)")
    ...
    def _create_sequences_strided(self, features, targets):
        ...
        if num_sequences <= 0:
            ...
            sequences = torch.zeros((1, self.sequence_length, num_features), 
...
def apply_undersampling(sequences: torch.Tensor, targets: torch.Tensor, strategy: str = "minority_match") -> tuple[torch.Tensor, torch.Tensor]:
...
def apply_undersampling_to_polar_dataset(dataset: PolarSequenceDataset, strategy: str = "minority_match") -> UnderSampledPolarDataset:
...
def print_dataset_distribution(dataset):
    ...
    if hasattr(dataset, 'y') and dataset.y is not None:
        ...
        logger.info("  Extracting targets from sequence dataset for distribution analysis...")
        targets_array = dataset.df[dataset.sequence_length:][dataset.target].to_numpy()
