...
class ModelTrainer:
    ...
    def __init__(self, config_dict: Dict[str, Any], device: str = None):
        ...
        self.learning_rate = float(config_dict.get("training", {}).get("learning_rate", 0.001))
        ...
        # Dataset parameters
        self.datasets_dir = config_dict.get("data", {}).get("prepared_datasets_dir", "data/prepared_datasets")
    ...
    def setup(self, datasets: Dict[str, Any], num_features: int, sequence_length: int):
        ...
        self.model_factory = ModelFactory(num_features, sequence_length, self.num_classes)
        
        logger.info(f"✓ Trainer setup complete - {num_features} features, sequence length {sequence_length}")
    ...
    def train_model(self, model_name: str) -> Dict[str, Any]:
        ...
        model = self.model_factory.create_model(model_name, **model_config).to(self.device)
        ...
        training_result = self._training_loop(
            ...
            optimizer, criterion, scheduler, scaler, model_name
        )
    ...
    def _create_data_loaders(self) -> tuple:
        ...
        data_on_cuda = sample_data[0].is_cuda if hasattr(sample_data[0], 'is_cuda') else False
        ...
        val_batch_size = min(self.batch_size * 4, 512)  # 4x larger for faster validation
        ...
        logger.info(f"Training batch size: {self.batch_size}, Validation batch size: {val_batch_size}")
    ...
    def _load_scaler(self) -> MinMaxScaler:
        ...
        if os.path.exists(scaler_path):
            ...
            logger.warning(f"⚠️ Scaler file not found at {scaler_path}, creating fallback scaler")
    ...
    def _training_loop(self, model, train_loader, validation_loader, 
                      optimizer, criterion, scheduler, scaler, model_name) -> Dict[str, Any]:
        ...
        for epoch in range(self.epochs):
            ...
            # Training phase
            avg_train_loss = self._train_epoch(model, train_loader, optimizer, criterion, epoch)
            
            # Validation phase
            avg_val_loss, val_accuracy = self._validate_epoch(model, validation_loader, criterion)
            ...
            if val_accuracy > best_accuracy:
                ...
                model_save_path = self._save_best_model(
                    model, optimizer, scaler, epoch, best_accuracy, 
                    best_loss, training_history, model_name
                )
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            if len(targets.shape) > 1:
                if not targets_fixed:
                    logger.info(f"Auto-fixing tensor shapes: {targets.shape} → 1D for CrossEntropyLoss")
                    targets_fixed = True
            ...
            if batch_idx % 100 == 0:
                logger.info(f"  Epoch {epoch+1}/{self.epochs}, Batch {batch_idx}/{len(train_loader)}, Loss: {loss.item():.4f}")
    ...
    def _save_best_model(self, model, optimizer, scaler, epoch, best_accuracy, 
                        best_loss, training_history, model_name) -> str:
        ...
        logger.info(f"  ✓ Best model saved: {model_save_path} (acc: {best_accuracy:.4f})")
        return model_save_path

def train_model(model_name: str, datasets: Dict[str, Any], num_features: int, 
               sequence_length: int, config_dict: Dict[str, Any], 
