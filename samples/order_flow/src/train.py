...
class TrainingOrchestrator:
    ...
    def load_datasets(self):
        ...
        try:
            ...
            logger.info(f"✓ Loaded datasets with {self.num_features} features, sequence length {self.sequence_length}")
            ...
            raise RuntimeError(f"Could not load datasets from {self.datasets_dir}. "
    ...
    def evaluate_model(self, model: nn.Module, split: str = 'test') -> Dict[str, Any]:
        ...
        batch_size = min(self.trainer.batch_size * 4, 512)  # Use larger batch size for evaluation
        return self.evaluator.evaluate_model(model, dataset, split, batch_size)


def main():
    """Main training function."""
    parser = argparse.ArgumentParser(description='Model Training with Pre-prepared Datasets')
    parser.add_argument('--config', type=str, default='configs/date_filtering_config.json', help='Configuration file')
    parser.add_argument('--datasets-dir', type=str, default='data/prepared_datasets', help='Directory containing prepared datasets')
    parser.add_argument('--model', type=str, choices=['RNN', 'CNN_LSTM', 'TFT', 'OrderFlow_Transformer'], 
                       default='RNN', help='Model to train')
    ...
    try:
        ...
        if not os.path.exists(args.config):
            ...
            config_dict = {
                "data": {
                    "prepared_datasets_dir": args.datasets_dir
                },
                "model": {
                    "hidden_size": 128,
                    "num_layers": 2
        ...
        if args.datasets_dir:
            config_dict.setdefault("data", {})["prepared_datasets_dir"] = args.datasets_dir
        if args.epochs:
