...
class PolarsDataProvider:
    ...
    def __init__(self, device, config_dict):
        ...
        self.feature_cols = config_dict["data"]["feature_cols"].split(",") if isinstance(config_dict["data"]["feature_cols"], str) else config_dict["data"]["feature_cols"]
        ...
        if self.instrument_filter:
            if isinstance(self.instrument_filter, str):
                self.instrument_filter = [x.strip() for x in self.instrument_filter.split(",")]
        ...
        self.lookahead_for_target_calculation = int(config_dict["data"]["lookforward_period"])
        self.buy_sell_threshold = int(config_dict["data"]["buy_sell_threshold"])
        self.sequence_length = int(config_dict["data"].get("sequence_length", self.lookback_period))
        
        # Undersampling configuration
        self.apply_undersampling = config_dict["data"].get("apply_undersampling", True)
        self.undersampling_strategy = config_dict["data"].get("undersampling_strategy", "minority_match")  # 'minority_match' or 'balanced'
    ...
    def load_and_prepare_data(self, save_datasets: bool = True, output_dir: str = "data/prepared_datasets") -> Dict[str, Any]:
        ...
        feature_df = feature_engine.compute_advanced_features(labeled_df, self.lookback_period)
    ...
    def _load_data_polars(self) -> pl.DataFrame:
        ...
        if self.metadata_location:
            logger.info(f"Loading data from metadata location: {self.metadata_location}")
            ...
            raise ValueError("Either metadata_location or data_path must be specified in configuration")
    ...
    def _load_from_iceberg_static(self) -> pl.DataFrame:
        ...
        try:
            ...
            logger.info(f"Loading Iceberg table from metadata using PyIceberg with predicate pushdown: {self.metadata_location}")
            ...
            if self.instrument_filter:
                logger.info(f"Applying instrument filter with predicate pushdown: {self.instrument_filter}")
            ...
            logger.info("Converting filtered scan to Arrow (memory efficient with predicate pushdown)...")
            # Convert filtered scan to Arrow (only loads filtered data from storage)
            arrow_table = scan.to_arrow()
            logger.info(f"✓ Retrieved {len(arrow_table)} rows from Iceberg using PyIceberg with predicate pushdown")
            ...
            if len(arrow_table) == 0:
                ...
                logger.info(f"Current filters: instrument={self.instrument_filter}, dates={self.start_date} to {self.end_date}")
                ...
                try:
                    ...
                    if len(debug_arrow) > 0:
                        ...
                        if 'instrument' in debug_arrow.column_names:
                            unique_instruments = debug_arrow.column('instrument').unique()
                            logger.info(f"Available instruments: {unique_instruments[:10].to_pylist()}")  # Show first 10
                        if 'trade_timestamp' in debug_arrow.column_names:
                            timestamp_col = debug_arrow.column('trade_timestamp')
                            logger.info(f"Date range in data: {timestamp_col.min()} to {timestamp_col.max()}")
            ...
            logger.info(f"✓ Successfully loaded {len(df)} rows using PyIceberg → Arrow → Polars pipeline")
            log_memory_usage("after Iceberg data loading")
            
            return df
            
        except Exception as e:
            logger.error(f"Failed to load from Iceberg using PyIceberg with predicate pushdown: {e}")
    ...
    def _load_from_iceberg_polars(self) -> pl.DataFrame:
        ...
        try:
            ...
            if '.' in path and not path.startswith('s3://'):
                logger.warning(f"Path {path} appears to be a catalog reference, but Polars needs direct metadata path")
                logger.warning("Please provide metadata_location in config or use direct S3 metadata path")
                raise ValueError("Polars requires direct S3 metadata path, not catalog reference")
            ...
            if self.instrument_filter:
                ...
                if len(self.instrument_filter) == 1:
                    lazy_df = lazy_df.filter(pl.col("instrument") == self.instrument_filter[0])
                else:
                    lazy_df = lazy_df.filter(pl.col("instrument").is_in(self.instrument_filter))
            ...
            logger.info(f"✓ Loaded {len(df)} rows from Iceberg table using Polars native support")
            return df
            
        except Exception as e:
            logger.error(f"Failed to load from Iceberg using Polars native support: {e}")
            logger.error("Note: Polars requires direct S3 metadata path, not catalog references")
            logger.error("Consider using metadata_location config option with direct S3 path")
            raise
    ...
    def _apply_iceberg_date_filters(self, scan):
        ...
        if self.date_filter_days:
            ...
            start_timestamp = datetime.strptime(self.start_date, "%Y-%m-%d").replace(tzinfo=timezone.utc)
            end_timestamp = (datetime.strptime(self.end_date, "%Y-%m-%d") + timedelta(days=1) - timedelta(microseconds=1)).replace(tzinfo=timezone.utc)
            scan = scan.filter(
                And(
                    GreaterThanOrEqual("trade_timestamp", start_timestamp.isoformat()),
                    LessThanOrEqual("trade_timestamp", end_timestamp.isoformat())
            ...
            start_timestamp = datetime.strptime(self.start_date, "%Y-%m-%d").replace(tzinfo=timezone.utc)
            scan = scan.filter(GreaterThanOrEqual("trade_timestamp", start_timestamp.isoformat()))
        elif self.end_date:
            end_timestamp = (datetime.strptime(self.end_date, "%Y-%m-%d") + timedelta(days=1) - timedelta(microseconds=1)).replace(tzinfo=timezone.utc)
            scan = scan.filter(LessThanOrEqual("trade_timestamp", end_timestamp.isoformat()))
    ...
    def _add_labels_polars(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns([
            ...
            pl.col('bid_price').shift(-lookahead).forward_fill().alias('future_bid_price'),
            pl.col('ask_price').shift(-lookahead).forward_fill().alias('future_ask_price'),
        ])
        ...
        df = df.with_columns([
            pl.when(pl.col('buy_target') > trigger_threshold * pl.col('bid_ask_spread'))
            .then(1)  # Buy
            .when(pl.col('sell_target') > trigger_threshold * pl.col('bid_ask_spread'))
            .then(2)  # Sell
            .otherwise(0)  # Hold
            .alias('target')
        ])
        ...
        for row in target_counts.iter_rows():
            ...
            label_name = {0: 'Hold', 1: 'Buy', 2: 'Sell'}.get(target_value, f'Label_{target_value}')
            percentage = (count / len(df)) * 100
            logger.info(f"  {label_name} ({target_value}): {count:,} ({percentage:.2f}%)")
        
        return df
    
    def _temporal_split_polars(self, df: pl.DataFrame) -> Tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame]:
    ...
    def _create_datasets_with_undersampling(self, train_df: pl.DataFrame, val_df: pl.DataFrame, test_df: pl.DataFrame) -> Dict[str, Any]:
        """Create PyTorch datasets using memory-efficient Polars processing."""
        logger.info("Creating PyTorch datasets with memory-efficient Polars processing...")
        log_memory_usage("before dataset creation")
        
        # Identify feature columns (exclude metadata columns)
        exclude_cols = ['trade_timestamp', 'trade_date', 'date', 'target', 'buy_target', 'sell_target', 
                       'future_bid_price', 'future_ask_price', 'bid_ask_spread',
                       'exchange_id', 'exchange_code', 'instrument', 'base_underlying_code',
                       'counter_underlying_code', 'tradeid', 'exchangetradetimestamp',
                       'quote_timestamp', 'side', 'bid', 'ask']
        ...
        logger.info("Scaling features using Polars MinMax scaling (memory efficient)...")
        ...
        if self.apply_undersampling:
            ...
            logger.info(f"✓ Applied undersampling to validation data for faster training")
    ...
    def _create_sequences_polars(self, df: pl.DataFrame, feature_cols: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        feature_mads = np.median(np.abs(features - feature_medians), axis=0, keepdims=True) 
        ...
        # Convert to tensors
        sequences_tensor = torch.tensor(sequences, dtype=torch.float32, device=self.device)
        targets_tensor = torch.tensor(aligned_targets, dtype=torch.long, device=self.device)
        
        return sequences_tensor, targets_tensor
    
    def _apply_undersampling(self, sequences: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        logger.info(f"Original distribution: {dict(zip(unique_targets.tolist(), target_counts.tolist()))}")
        ...
        if self.undersampling_strategy == "minority_match":
            ...
            raise ValueError(f"Unknown undersampling strategy: {self.undersampling_strategy}")
        ...
        logger.info(f"Final distribution after undersampling: {dict(zip(final_unique.tolist(), final_counts.tolist()))}")
        logger.info(f"Reduced from {len(sequences)} to {len(undersampled_sequences)} sequences")
        
        return undersampled_sequences, undersampled_targets
    
    
    def _create_sequence_dataset(self, df: pl.DataFrame, features: List[str], target: str):
        """Create a truly memory-efficient sequence dataset from Polars DataFrame (NO pandas conversion)."""
        logger.info("Creating sequence dataset (memory efficient - no pandas conversion)...")
    ...
    def _apply_undersampling_to_sequence_dataset(self, dataset):
        ...
        if isinstance(dataset, PolarSequenceDataset):
            return apply_undersampling_to_polar_dataset(dataset, self.undersampling_strategy)
        ...
        undersampled_sequences, undersampled_targets = apply_undersampling(sequences, targets, self.undersampling_strategy)
    ...
    def _print_dataset_distribution(self, dataset):
        ...
        if hasattr(dataset, 'y') and dataset.y is not None:
            ...
            logger.info("  Extracting targets from streaming dataset for distribution analysis...")
        ...
        elif hasattr(dataset, 'original_dataset') and isinstance(dataset.original_dataset, PolarSequenceDataset):
            ...
            targets_array = original_dataset.df[original_dataset.sequence_length:][original_dataset.target].to_numpy()
        ...
        for split_name, dataset in datasets.items():
            ...
            torch.save({
                ...
                'metadata': {
                    ...
                    'undersampling_applied': self.apply_undersampling and split_name == 'train',
                    'undersampling_strategy': self.undersampling_strategy if self.apply_undersampling else None
                }
            }, dataset_path)
            
            logger.info(f"✓ Saved {split_name} dataset: {dataset_path} ({len(dataset):,} samples)")
        ...
        for split_name in ['train', 'validation', 'test']:
            ...
            if dataset_file.exists():
                ...
                if metadata.get('undersampling_applied'):
                    logger.info(f"    Undersampling: {metadata.get('undersampling_strategy', 'unknown')}")
            else:
...
def prepare_data_pipeline(config_dict: Dict[str, Any], save_datasets: bool = True, output_dir: str = "data/prepared_datasets") -> Dict[str, Any]:
    ...
    # Run pipeline
    datasets = data_provider.load_and_prepare_data(save_datasets=save_datasets, output_dir=output_dir)
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/date_filtering_config.json', help='Configuration file')
    parser.add_argument('--output-dir', type=str, default='data/prepared_datasets', help='Output directory for datasets')
    parser.add_argument('--no-save', action='store_true', help='Do not save datasets to disk')
