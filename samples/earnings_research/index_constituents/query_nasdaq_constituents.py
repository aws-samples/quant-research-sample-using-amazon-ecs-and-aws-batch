#!/usr/bin/env python
"""Query Nasdaq-100 constituents by date, fetching on-demand if not cached.

Usage:
    scripts/.venv/bin/python scripts/query_nasdaq_constituents.py 2026-08-06
    scripts/.venv/bin/python scripts/query_nasdaq_constituents.py 2026-08-06 --check CPOP

Returns the Nasdaq-100 constituent list for the given date. If not cached, fetches
...
def get_constituents(date: str, profile: Optional[str] = None) -> pd.DataFrame:
    """Get Nasdaq-100 constituents for date, fetching if not cached."""
    ...
    # Not cached - fetch it
    print(f"Not cached, fetching Nasdaq-100 {date}...", file=sys.stderr)
    fetch_script = Path(__file__).parent / "fetch_nasdaq_constituents.py"
...
def main():
    ...
    if args.check:
        ...
        if ticker_upper in df["ticker"].values:
            print(f"\n✓ {ticker_upper} IS in Nasdaq-100 on {args.date}")
            row = df[df["ticker"] == ticker_upper].iloc[0]
            print(f"  Company: {row['company_name']}")
        else:
            print(f"\n✗ {ticker_upper} is NOT in Nasdaq-100 on {args.date}")
        return 0

    # Print full list
    print(f"\nNasdaq-100 Constituents on {args.date} ({len(df)} total):\n")
