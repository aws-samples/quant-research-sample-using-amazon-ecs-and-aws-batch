#!/usr/bin/env python
"""Fetch and cache Russell 1000 constituents by date.

Usage:
    scripts/.venv/bin/python scripts/fetch_russell_1000_constituents.py 2026-08-10
    scripts/.venv/bin/python scripts/fetch_russell_1000_constituents.py --list

Source: iShares Russell 1000 ETF (IWB) holdings via BlackRock.
Caches to S3: s3://{bucket}/russell-1000-constituents/date={date}/weights.csv
Updates local catalog: <constituent_cache_dir>/russell_1000_constituents_cache.json
...
S3_PREFIX = "russell-1000-constituents"
def _cache_file() -> Path:
    return settings.root() / settings.get("paths", "constituent_cache_dir") / "russell_1000_constituents_cache.json"

# iShares Russell 1000 ETF (IWB)
ISHARES_PRODUCT_ID = "239707"
ISHARES_TICKER = "iwb"
...
def fetch_constituents(date: dt_date) -> pd.DataFrame:
    """Fetch Russell 1000 constituents from iShares IWB ETF.
    ...
    # iShares CSV structure:
    # Lines 1-9: metadata (fund name, date, stats)
    # Line 10: header row (Ticker, Name, Sector, ...)
    # Lines 11+: holdings data
    ...
    # Remove any duplicate tickers (shouldn't happen but defensive)
    result = result.drop_duplicates(subset=['ticker'], keep='first')

    if len(result) < 900:
        raise ValueError(f"Only found {len(result)} holdings, expected ~1000 for Russell 1000")
...
def load_cache() -> dict:
    ...
    if not _cache_file().exists():
        return {"name": "Russell 1000", "source": "iShares IWB ETF", "dates": {}}
...
def main():
    ...
    # Fetch and cache
    print(f"Fetching Russell 1000 constituents for {fetch_date}...")
    print(f"Source: iShares IWB ETF (product {ISHARES_PRODUCT_ID})")
    try:
